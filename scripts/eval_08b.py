#!/usr/bin/env python3
"""Qwen3.5-0.8B Memory Oracle 评测（200 样本，F1 比较）

用法：
    python scripts/eval_08b.py

固定路径：
    BASE_MODEL  = /home/shuzuan/models/Qwen/Qwen3___5-0.8B
    CKPT_DIR    = /home/shuzuan/prj/titans-pytorch-mlx/checkpoints/oracle_08b/stage3/final
...
BASE_MODEL = "/home/shuzuan/models/Qwen/Qwen3___5-0.8B"
CKPT_DIR   = "/home/shuzuan/prj/titans-pytorch-mlx/checkpoints/oracle_08b/stage3"
EVAL_DATA  = "/home/shuzuan/prj/titans-pytorch-mlx/data/oracle_eval.jsonl"
DEVICE     = "cuda:7"
MAX_SAMPLES = 200
...
def main():
    ...
    logger.info("Loading MemoryOracle (0.8B) from %s ...", CKPT_DIR)
    ...
    # 用 bad_words_ids 屏蔽 <think> token（ID=248068），避免 thinking 模式输出空结果
    _THINK_ID = 248068
    import types
    ...
    for idx, sample in enumerate(samples):
        ...
        if (idx + 1) % 20 == 0 or idx == 0:
            ...
            logger.info(
                "[%3d/%d] F1: write=%.3f  no-write=%.3f  Δ=%.3f",
                idx + 1, len(samples), avg_w, avg_nw, avg_w - avg_nw,
            )
            logger.info("  query : %s", query[:80])
            logger.info("  gold  : %s", gold_answer[:80])
            logger.info("  write : %s", pred_write[:80])
            logger.info("  nowr. : %s", pred_nowrite[:80])
    ...
    print("\n" + "=" * 60)
    print(f"Qwen3.5-0.8B Memory Oracle 评测结果（{len(samples)} 样本）")
